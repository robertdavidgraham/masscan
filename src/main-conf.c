...
{
    enum OutputFormat x = Output_Default;
    UNUSEDPARM(name);
    if (masscan->echo) {
 ...
 ***************************************************************************/
void
masscan_command_line(struct Masscan *masscan, int argc, const char *argv[])
{
    int i;
...
mainconf_selftest()
{
    ...
    {
        int argc = 6;
        const char *argv[] = { "foo", "bar", "-ddd", "--readscan", "xxx", "--something" };
